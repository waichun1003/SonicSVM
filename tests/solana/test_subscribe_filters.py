...
class TestSubscribeFilters:
    ...
    async def test_subscribe_multiple_programs(
        ...
        async with solana_stream_route.client(timeout=40) as ws:
            ...
            messages = await ws.collect_messages(count=1, timeout=30)
            non_hello = [m for m in messages if m.get("type") != "stream_hello"]
            assert len(non_hello) > 0, "No data with multiple program filters"

    @pytest.mark.xfail(
        reason="Stream delivery is intermittent and may not produce data in a 60s window",
        strict=True,
    )
    @pytest.mark.finding
    async def test_any_non_hello_message_within_60s(
        ...
        """After subscribing, non-hello messages should arrive within 60s."""
        async with solana_stream_route.client(timeout=70) as ws:
            hello = await ws.recv_json(timeout=10)
            assert hello["type"] == "stream_hello"
